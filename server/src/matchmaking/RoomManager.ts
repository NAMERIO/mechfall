...
export class RoomManager {
  ...
  findGame(requestedGameId?: string): FindGameResult {
    ...
    if (gameId) {
      if (!/^[A-Z0-9]{6}$/.test(gameId)) return { error: "invalid_game_id" };
      room = this.rooms.get(gameId);
      if (!room) return { error: "invalid_game_id" };
      if (room.full) return { error: "full" };
    } else {
      room = [...this.rooms.values()].find((candidate) => !candidate.full);
    }
    if (!room) {
      room = new GameRoom();
    ...
    this.tickets.set(ticket, { gameId: room.id, expiresAt: Date.now() + 30_000 });
